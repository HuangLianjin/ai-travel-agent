...
class Database:
    ...
    def init_db(self) -> None:
        tables = [
            ...
            """CREATE TABLE IF NOT EXISTS user_feedback (
                    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    user_id BIGINT NOT NULL,
                    target_type TEXT NOT NULL DEFAULT 'trip',
                    target_id TEXT NOT NULL DEFAULT '',
    ...
    def _decode_user_feedback(self, row: dict | None) -> dict | None:
        if not row:
            return None
        result = dict(row)
        try:
    ...
    def save_user_feedback(
        ...
        feedback_id = self._insert_id(
            "INSERT INTO user_feedback "
            "(user_id, target_type, target_id, rating, tags, comment, status, created_at) "
        ...
        low_score = rating <= 2
        if low_score:
            self.save_eval_failures(
                f"feedback-{feedback_id}",
                [
                    {
                        "case_id": target_id or f"feedback-{feedback_id}",
                        "title": "用户低分反馈",
                        "input_text": comment or f"{target_type} 用户评分 {rating}/5",
                        "failure_types": ["user_low_rating"],
                        "quality_failures": [],
    ...
    def list_user_feedback(
        ...
        if user_id is None:
            ...
            sql = (
                "SELECT * FROM user_feedback WHERE user_id = %s "
                "ORDER BY created_at DESC, id DESC LIMIT %s OFFSET %s"
            )
            params = (user_id, limit, offset)
        return [
            self._decode_user_feedback(row) or {}
            for row in self.query_all(sql, params)
        ]

    def user_feedback_summary(self, days: int = 30) -> dict:
        cutoff = (
            datetime.now(timezone.utc) - timedelta(days=max(1, int(days)))
        ).isoformat(timespec="seconds")
        rows = self.query_all(
            "SELECT target_type, rating FROM user_feedback WHERE created_at >= %s",
            (cutoff,),
        )
        ratings = [int(row.get("rating") or 0) for row in rows]
        by_target: dict[str, int] = {}
        for row in rows:
            target = str(row.get("target_type") or "unknown")
